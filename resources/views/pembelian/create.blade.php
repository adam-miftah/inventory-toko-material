    ...
    <div class="container-fluid">
        <div class="page-header mb-4">
            <div class="d-flex justify-content-between align-items-center">
                <h4 class="page-title">
                    <i class="fas fa-truck-loading me-2"></i> Pembelian Baru
                </h4>
            </div>
        </div>
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                        <form action="{{ route('pembelian.store') }}" method="POST">
                                                        ...
                                                        <div class="col-md-4">
                                                            ...
                                                            <select
                                                                ...
                                                                @foreach ($items as $item)
                                                                    <option value="{{ $item->id }}" data-name="{{ $item->name }}" data-price="{{ $item->price }}" {{ old('items.' . $key . '.item_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                                                @endforeach
                                                            </select>
                            ...
                            <button type="submit" class="btn btn-outline-primary btn-sm">Simpan Pembelian</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            $(document).ready(function () {
                ...
                const allItems = @json($items); // Pass all items to JavaScript

                function generateItemRow(index) {
                    let options = '<option value="">Pilih Produk</option>';
                    allItems.forEach(item => {
                        options += `<option value="${item.id}" data-name="${item.name}" data-price="${item.price}">${item.name}</option>`;
                    });
                    ...
                    return `
                        <div class="card mb-2" id="item-${index}">
                            <div class="card-body">
                                <div class="row g-3 align-items-center">
                                    <div class="col-md-4">
                                        <label for="item_id_${index}" class="form-label">Produk</label>
                                        <select class="form-select item-select @error('items.${index - 1}.item_id') is-invalid @enderror" id="item_id_${index}" name="items[${index}][item_id]" required>
                                            ${options}
                                        </select>
                                        @error('items.${index - 1}.item_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-3">
                                        <label for="quantity_${index}" class="form-label">Kuantitas</label>
                                        <input type="number" class="form-control quantity-input @error('items.${index - 1}.quantity') is-invalid @enderror" id="quantity_${index}" name="items[${index}][quantity]" value="1" min="1" required>
                                        @error('items.${index - 1}.quantity')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-3">
                                        <label for="price_${index}" class="form-label">Harga Satuan</label>
                                        <input type="number" class="form-control price-input" id="price_${index}" name="items[${index}][price]" readonly>
                                    </div>
                                    <div class="col-md-2 text-end">
                                        <button type="button" class="btn btn-sm btn-danger remove-item" data-item-id="${index}">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </div>
                                </div>
                ...
                $('#add-item').click(function () {
                    itemCount++;
                    $('#purchase-items-container').append(generateItemRow(itemCount));
                    initiateSearchableSelect(`#item_id_${itemCount}`);
                });
                ...
                function initiateSearchableSelect(selector) {
                    $(selector).select2({
                        ...
                        const itemId = $(this).attr('id').split('_')[2];
                        $(`#price_${itemId}`).val(data.price);
                    });
                }

                // Initialize searchable select for existing items on page load
                $('.item-select').each(function() {
                    initiateSearchableSelect(`#${$(this).attr('id')}`);
                });
            });
        </script>
    @endpush
